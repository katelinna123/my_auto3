...
@pytest.mark.smoke
@pytest.mark.p0
@pytest.mark.parametrize('card_number', DATA['test_add_fuel_card'])
...
def test_add_fuel_card_exist(db):
    card_number = DATA['test_add_fuel_card_exist']
...
def test_add_fule_card_twice(db):
    card_numbers = DATA['test_add_fule_card_twice']
...
def test_add_fule_card_3times(db):
    card_numbers = DATA['test_add_fule_card_3times']
    for card_number in card_numbers:
        url = 'http://115.28.108.130:8080/gasStation/process'
        data = {"dataSourceId": "bHRz", "methodId": "00A", "CardInfo": {"cardNumber": card_number}}
        res = requests.post(url, json=data).json()
...
def test_add_fule_card_wrong_request_format():
    card_number = DATA['test_add_fule_card_wrong_request_format']
    url = 'http://115.28.108.130:8080/gasStation/process'
    data = {"dataSourceId": "bHRz", "methodId": "00A", "CardInfo": {"cardNumber": card_number}}
